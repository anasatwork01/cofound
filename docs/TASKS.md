...
| 0.3  | `packages/schema`: JSON Schema + OpenAPI sources, codegen to TS/Go/Python, `make gen`, CI drift check                                                                                                                                                                                                                                        | 0.1                  | todo     |
| 0.4  | Go service chassis: config, slog JSON logging, OTel tracing, health/readiness, graceful shutdown, chi router, error envelope                                                                                                                                                                                                                 | 0.2, §21.2           | blocked  |
| 0.5  | Python service chassis: FastAPI app factory, settings, logging, OTel, health endpoints                                                                                                                                                                                                                                                       | 0.2, §21.2           | blocked  |
| 0.6  | Control plane schema: goose migrations for all of SPEC §6, pgx pool, RLS policies + per-transaction `app.org_id`, integration test proving cross-tenant denial. Write the policy with `nullif(current_setting('app.org_id', true), '')::uuid` — see `docs/verified.md`. Also turn on Go build caching in CI, which needs a `go.sum` to exist | 0.4                  | todo     |
| 0.7  | Auth: email magic link + Google OAuth, rotating httpOnly `SameSite=Lax` sessions                                                                                                                                                                                                                                                             | 0.6                  | todo     |
| 0.8  | Tenancy middleware: resolve `(user_id, org_id, role)` once, role matrix in one place, never inline                                                                                                                                                                                                                                           | 0.7                  | todo     |
| 0.9  | Org + project CRUD, invites, org switching, `audit_log` writer covering SPEC §8's list                                                                                                                                                                                                                                                       | 0.8                  | todo     |
| 0.10 | `Idempotency-Key` middleware, structured error contract, edge rate limiting                                                                                                                                                                                                                                                                  | 0.4                  | todo     |
...
| 1.5  | Warm pool (2-5 per popular image, scaled on rolling p50), Redis registry, quota enforcement                                                                | 1.3           | todo   |
...
| 1.9  | opencode patches P4 (provider base URL forced to `aigw`, local credentials disabled) and P1 (per-turn usage telemetry over a Unix socket)                  | 1.1           | todo   |
| 1.10 | opencode patches P5 (`onTurnStart`/`onTurnEnd` hooks), P6 (structured event stream matching SPEC §7.2), P3 (non-overridable policy from `$HALYARD_POLICY`) | 1.1, 0.3      | todo   |
...
| 1.13 | `aigw` v1: provider proxy behind config, exact four-way token accounting, per-session spend caps (no caching yet)                                          | 0.4, §22.13   | todo   |
...
_Accept: create from template in <10s p50; browse files with no sandbox
running; edit as a human without racing the agent; restore any version and see
...
| 4.6  | Wire reserve/settle into the turn lifecycle; patch P2 for mid-stream budget abort; bounded negative float so an overrun blocks the _next_ turn instead of killing the running one | 4.5, 1.10, 1.15 | todo    |
