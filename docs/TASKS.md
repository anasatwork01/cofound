...
| 0.2  | CI: GitHub Actions running `make verify` per language, plus Postgres + Redis in Docker for integration tests                                                   | 0.1                  | todo     |
...
| 0.6  | Control plane schema: goose migrations for all of SPEC §6, pgx pool, RLS policies + per-transaction `app.org_id`, integration test proving cross-tenant denial | 0.4                  | todo     |
