...
| 0.5  | Python service chassis: FastAPI app factory, settings, logging, OTel, health endpoints                                                                                                                                                                                  | 0.2, §21.2           | todo     |
| 0.6  | Control plane schema: goose migrations for all of SPEC §6, pgx pool, RLS policies + per-transaction `app.org_id`, integration test proving cross-tenant denial. Write the policy with `nullif(current_setting('app.org_id', true), '')::uuid` — see `docs/verified.md`. | 0.4                  | todo     |
