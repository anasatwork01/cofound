...
The flow itself now holds, verified in a real browser against a real API on a
second origin (task 0.14). What is left of the sentence is "on a real deployed
URL", which is 0.12's remaining half and waits on §21 decision 1.
...
| 0.14 | Console to API integration: sign-in (magic link + Google), session-aware routing on `/`, org creation, the project list bound to `GET /projects` with its `X-Halyard-Org` header, and project creation from the prompt box. Adds three routes §18 does not list - `/signin`, `/auth/callback` (the address the API builds into every emailed link) and `/orgs/new` - argued in `src/lib/routes.ts`. A template card still needs a name field to create with. Also the four things a browser found that no test did: **CORS** on the api (no console fetch left Chromium without it), a **numbered placeholder name** so a second prompt project is not a 409, `common.Timestamp`'s **JSON methods** (a defined type over `time.Time` encoded as `{}`), and both **axe route lists derived** from `lib/routes.ts` rather than hand-written | 0.7, 0.9, 0.11       | **done**    |
